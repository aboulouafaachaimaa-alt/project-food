...
import { Menu, X, ShoppingCart, Star, ChefHat, Clock, MapPin, Phone } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-500 ${
        scrolled ? 'bg-black/95 backdrop-blur-md shadow-2xl shadow-amber-900/20 py-3' : 'bg-transparent py-6'
      }`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className={`font-serif text-3xl bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent transition-all duration-500 ${
          ...
          <div className="hidden md:flex items-center gap-8">
            {['HOME', 'ABOUT', 'MENU', 'RESERVATIONS', 'CONTACT'].map((item, idx) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="relative text-sm tracking-wider text-slate-300 hover:text-amber-400 transition-all duration-300 group"
            ...
            <button className="bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-2.5 rounded-full hover:shadow-lg hover:shadow-amber-900/50 hover:scale-105 transition-all duration-300 flex items-center gap-2">
          ...
          <div className="bg-slate-900/95 backdrop-blur-md px-6 py-4 space-y-4">
            {['HOME', 'ABOUT', 'MENU', 'RESERVATIONS', 'CONTACT'].map(item => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="block text-sm tracking-wider text-slate-300 hover:text-amber-400 transition-colors duration-300"
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          <div className={`transition-all duration-1000 ${
            ...
            <button className="group bg-gradient-to-r from-amber-600 to-orange-600 px-10 py-4 rounded-full text-lg font-semibold hover:shadow-2xl hover:shadow-amber-900/50 hover:scale-105 transition-all duration-300 inline-flex items-center gap-3">
              EXPLORE OUR MENU
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-2 rounded-full text-sm hover:shadow-lg hover:shadow-amber-900/50 transition-all duration-300 hover:scale-105">
                      Order Now
                    </button>
                  </div>
                </div>
      ...
      <section className="py-20 bg-gradient-to-br from-slate-900 via-amber-900/20 to-slate-900 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <button className="bg-gradient-to-r from-amber-600 to-orange-600 px-12 py-4 rounded-full text-lg font-semibold hover:shadow-2xl hover:shadow-amber-900/50 hover:scale-105 transition-all duration-300">
            MAKE A RESERVATION
          </button>
        </div>
      </section>
      ...
      <footer className="bg-black py-12 border-t border-amber-900/30">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="text-center pt-8 border-t border-amber-900/30">
            <p className="text-slate-500 text-sm">© 2024 Savory. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
